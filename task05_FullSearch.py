...
# List of CAS numbers for compounds to analyze. The program will automatically check if the corresponding template files exist in the ./templates folder, and skip the compound if the template is not found.
cpd_aim_ls = ['7004-03-7', '56-41-7', '98-79-3', '70-26-8', '50-21-5', '56-12-2', 'anomeric-7660-25-5', '62-49-7', '58-61-7']

template_ids = {
    os.path.splitext(os.path.basename(p))[0]
    for p in files_template
}
cpd_aim_ls_valid = []
for cpd in cpd_aim_ls:
    if cpd in template_ids or f'anomeric-{cpd}' in template_ids:
        cpd_aim_ls_valid.append(cpd)
    else:
        print(f'Skip {cpd}: template not found in ./templates')
cpd_aim_ls = cpd_aim_ls_valid
...
for cpd_aim in cpd_aim_ls:
    ...
    if data_aim.shape[1] == 6:
        ...
        output_sugar = sugar(
            cpd_aim=cpd_aim_, cpd_name=cpd_name, cpd_m=cpd_m, data_aim=data_aim,
            thres_=threshold, rw_=rw, df_sample_=df_sample, step_=step, bl_=bl
        )
        if output_sugar is None:
    ...
    for cx in clusters:
        ...
        if fitting_cx is None:
            for p_ind in peak_indices:
                fit_peaks.append(fit_range.iloc[p_ind, 0])
        ...
        if (px < 5) and (px > 1):
            if len(peak_indices) > px:
                ...
                if len(peak_key_cx) == px:
                    ...
                    i_comb = [i_c for i_c in range(0, len(peak_key_cx))]
                    ...
                    for comb_ in comb:
                        if peak_max_index in comb_:
                            cos_peaks = 1 - spatial.distance.cosine(
                                peak_heights_norm[list(comb_)], peak_heights_t_norm
                            )
                    ...
                    for i_ in comb_final:
                        ...
                        fit_peaks.append(fit_range.iloc[peak_indices[i_], 0])
        ...
        for comb in all_combinations:
            ...
            if len(out_re) > 0:
                ...
                for p_re_ind in peak_ind_re:
                    peak_ppm_re_ls.append(out_range.iloc[p_re_ind, 0])

                output_re = [
                    cpd_name, cpd_aim, peak_ppm_re_ls, cx_re,
                    data_aim.loc[data_aim['cluster'] == cx_re, 'Hs'].values[0],
...
output_path = f'./0624quantified-FS-{name_sample}.xlsx'
try:
    output_df.to_excel(output_path)
except PermissionError:
    ts = datetime.now().strftime('%Y%m%d-%H%M%S')
    output_path = f'./0624quantified-FS-{name_sample}-{ts}.xlsx'
    output_df.to_excel(output_path)
